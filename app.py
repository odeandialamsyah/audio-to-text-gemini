...
def save_uploaded_file(uploaded_file):
    """
    Menyimpan file upload Streamlit ke temporary file lokal.
    Ini diperlukan karena Gemini Files API membutuhkan path file.
    """
...
def wait_for_file_active(client, uploaded_file, timeout_seconds=300):
    """
    Menunggu file selesai diproses oleh Gemini Files API.
    Penting untuk file besar seperti audio 80 MB.
    """
    ...
    while file.state.name == "PROCESSING":
        if time.time() - start_time > timeout_seconds:
            raise TimeoutError("File terlalu lama diproses oleh Gemini. Coba ulangi atau kompres audio.")
...
def build_prompt(language: str, clean_text: bool, speaker_label: bool, timestamp_mode: bool) -> str:
    """
...
def transcribe_audio(
    ...
    """
    Upload audio ke Gemini Files API, tunggu aktif,
...
with st.sidebar:
    ...
    st.caption("Untuk audio panjang, aktifkan speaker label jika rekaman berisi percakapan.")


uploaded_audio = st.file_uploader(
    "Upload file audio",
    type=["mp3", "wav", "m4a", "ogg", "flac", "mpeg", "mpga", "aac"]
)
